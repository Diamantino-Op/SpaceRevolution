...
public class ElectricCableTickManager {
    private static final List<ElectricCableBlockEntity> cableList = new ArrayList<>();
    private static final List<OfferedEnergyStorage> targetStorages = new ArrayList<>();
    private static final Deque<ElectricCableBlockEntity> bfsQueue = new ArrayDeque<>();
    ...
    static void handleCableTick(ElectricCableBlockEntity startingCable) {
        ...
        try {
            ...
            for (ElectricCableBlockEntity cable : cableList) {
    ...
    private static boolean shouldTickCable(ElectricCableBlockEntity current) {
     ...
     */
    private static void gatherCables(ElectricCableBlockEntity start) {
        ...
        while (!bfsQueue.isEmpty()) {
            ElectricCableBlockEntity current = bfsQueue.removeFirst();

            for (Direction direction : Direction.values()) {
                if (current.getAdjacentBlockEntity(direction) instanceof ElectricCableBlockEntity adjCable && current.getCableType().transferRate == adjCable.getCableType().transferRate) {
     ...
     */
    private static long dispatchTransfer(CableVariants.Electric cableType, TransferOperation operation, long maxAmount) {
        // Build target list.
